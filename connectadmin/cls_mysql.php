...
class cls_mysql
{
    ...
    var $cache_data_dir = 'caches/mysql/';
    var $root_path      = '';
    ...
    function connect($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 0)
    {
        ...
        if ($this->max_cache_time && $this->starttime > $this->mysql_config_cache_file_time + $this->max_cache_time)
        {
            ...
            @$this->file_put_contents($sqlcache_config_file, $content);
        }
    ...
    function query($sql, $type = '')
    {
        ...
        if (defined('DEBUG_MODE') && (DEBUG_MODE & 8) == 8)
        {
            ...
            if (PHP_VERSION >= '5.0')
            {
                $this->file_put_contents($logfilename, $str, FILE_APPEND);
            }
            else
            {
    ...
    function escape_string($query, $unescaped_string)
    {
        if (PHP_VERSION >= '4.3')
        {
            // return mysql_real_escape_string($unescaped_string);
            return mysqli_real_escape_string($query, $unescaped_string);
        }
        else
        {
    ...
    function setSqlCacheData($result, $data)
    {
        ...
        {
            @$this->file_put_contents($result['filename'], '<?php exit;?>' . time() . serialize($data));
            clearstatcache();
        }
    }

    /* 獲取 SQL 語句中最後更新的表的時間，有多個表的情況下，返回最新的表的時間 */
